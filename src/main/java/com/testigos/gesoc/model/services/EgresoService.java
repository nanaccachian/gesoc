...
//    }
    ...
    public List<Egreso> findAllConProveedoreItems(Entidad entidad) {
        return repo.findAllConProveedoreItems().stream().filter(e -> e.getComprador().getId() == entidad.getId()).collect(Collectors.toList());
    }
    ...
    public Double montoActual(Entidad entidad) {
        return repo
                .findAllConItems()
                .stream()
                .filter(e -> e.getComprador().getId() == entidad.getId())
                .mapToDouble(Egreso::valorTotal)
                .sum();
    }
    ...
    public Double montoMes(Entidad entidad) {
        return repo
                .findAllConItems()
                .stream()
                .filter(e -> e.getFechaOperacion().getMonthValue() == LocalDate.now().getMonthValue() && e.getComprador().getId() == entidad.getId())
                .mapToDouble(Egreso::valorTotal)
                .sum();
    }
    ...
    public Double montoAnio(Entidad entidad) {
        return repo
                .findAllConItems()
                .stream()
                .filter(e -> e.getFechaOperacion().getYear() == LocalDate.now().getYear() && e.getComprador().getId() == entidad.getId())
                .mapToDouble(Egreso::valorTotal)
                .sum();
    }
